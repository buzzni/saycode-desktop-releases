...
**기업용 바이브 코딩 & 배포 플랫폼 (Enterprise Vibe Coding & Delivery Platform)** —
만들고 싶은 것을 말로 설명하면 AI 에이전트가 만들고 실시간으로 보여주며,
그 결과물은 회사가 배포하고, 공유하고, 소유합니다.
...
### [⬇️ macOS용 다운로드 (Apple Silicon)](https://github.com/buzzni/saycode-desktop-releases/releases/download/v0.1.40/Saycode-0.1.40-arm64.dmg)
...
- [최신 릴리스 노트](docs/releases/v0.1.40.ko.md)
...
| AI 토큰 비용 | 매번 가장 비싼 모델 사용 | **난이도 기반 자동 라우팅으로 턴당 최대 ~90% 절감** |
| 다중 에이전트 운영 | 채팅 하나씩 알트탭 | **칸반 에이전트 보드로 함대 전체를 한 화면에서 지휘** |
...
|---|---|---|
| 주된 초점 | 개발자의 코딩 루프 | **회사의 딜리버리 루프** — 제작 → 배포 → 공유 → 인수인계 |
| 관리 단위 | Repo · worktree · 에이전트 세션 | **조직 · 팀 · 프로젝트 · 머신 · 배포 결과물** |
| "배포"의 의미 | 에이전트 도구를 조직에 안전하게 도입·확산 | **만든 결과물을 팀이 여는 사내 URL로 서비스** |
| 일반적인 결승선 | 리뷰를 거쳐 Git에 병합된 변경 | **공유·인수인계 가능한, 실제 운영 중인 사내 서비스** |
| AI 모델 비용 | 사용량 추적, 계정 전환 | **난이도 기반 자동 라우팅 + 서브 에이전트 위임 — 조직 전체 ~50% 절감 사례** |
...
- **비용 운영** — 요청 난이도에 따라 가장 저렴한 적정 모델로 자동 라우팅하고(세션이
  ...
  위임하며, 이 모든 것이 중앙에서 관리되는 조직 공용 머신 위에서 돌아갑니다. 이렇게
  운영한 조직은 전체 AI 토큰 비용을 50% 가까이 절감하기도 했습니다 — 메시지마다 어떤
  모델이 왜 선택됐는지 배지로 표시됩니다. 회사 전체의 AI 비용을 "확인"이 아니라
  "운영"합니다.
...
### 🔌 스탠드얼론 모드 — 완전 로컬, 설정 제로

계정이 없어도 됩니다. 로그인 화면에서 **로컬 전용 모드 (Standalone)** 를 한 번 클릭하면
Saycode가 **자체 내장 서버**를 띄우고, 내 머신을 등록하고, 완전한 워크스페이스로 바로
데려다줍니다 — 몇 초 만에요. 릴레이도, 데이터베이스도, 데이터 한 바이트까지 전부 내 Mac
안에 있고, 인터넷에 아무것도 나가지 않습니다. 보안에 민감한 팀, 폐쇄망 환경, 혹은 설치
후 1분 안에 Saycode의 모든 걸 직접 굴려보고 싶은 분께 딱입니다. 팀 기능이 필요해지면
그때 로그인하고 이어가면 됩니다.

</td>
<td><img src="docs/assets/standalone-mode.gif" alt="로컬 전용 모드 클릭 한 번으로 내장 서버가 뜨고 완전 로컬 워크스페이스가 열립니다" /></td>
</tr>
<tr>
<td width="42%" valign="middle">
...
정말 어려운 문제는 상위 모델로 — 그리고 세션이 진짜로 막혔을 때만 가장 강력한(가장
비싼) 티어로 승격합니다. 모든 메시지에 *"⚡ 자동 선택 · Haiku 4.5 · ~90% 절약"* 같은
투명성 배지가 붙고, 세션 헤더에 누적 절약률이 집계되어 — 얼마를 아꼈는지, 왜 그 모델이
선택됐는지 항상 알 수 있습니다. 회사 단위에서는 효과가 더 커집니다: 기계적인 하위
작업을 가벼운 모델의 서브 에이전트에게 위임하고 조직 공용 머신에서 사용량을 중앙
관리하면, 전체 AI 토큰 비용을 50% 가까이 절감하기도 합니다.

</td>
<td><img src="docs/assets/auto-model-routing.gif" alt="쉬운 요청은 저렴한 모델로, 어려운 요청은 상위 모델로 자동 라우팅되고 턴별 절약 배지가 붙습니다" /></td>
</tr>
<tr>
<td width="42%" valign="middle">
...
보관하세요 — **Commit & PR**과 **PR 머지**도 카드에서 바로 실행됩니다. 자동 선택이
아껴준 비용은 일간/주간/월간 위젯으로 집계됩니다.

</td>
<td><img src="docs/assets/agent-board.gif" alt="에이전트 보드 칸반: 세션 카드를 리뷰 중 컬럼으로 드래그해 코드 리뷰를 요청" /></td>
</tr>
<tr>
<td width="42%" valign="middle">
...
반영한 뒤 그대로 **Commit & PR**로 이어집니다. 리뷰 이력과 발견 사항은 세션에 함께
남습니다.

</td>
<td><img src="docs/assets/work-completion-hub.png" alt="작업 마무리 허브: 현재 에이전트 리뷰, 독립 리뷰어 위임, 바로 Commit &amp; PR" /></td>
</tr>
<tr>
<td width="42%" valign="middle">
...
검색합니다. 내 머신을 떠나지 않는 로컬 SQLite FTS5 인덱스 기반입니다. *내 대화*와
*조직 전체* 범위를 오가고, `project:web`, `role:agent`, `after:2026-07-01`,
`"정확한 문구"` 같은 필터로 좁혀 보세요.
...
- 🧭 **첫 실행 온보딩 가이드** — 내장 서버 확인, Claude/Codex 로그인 자동 감지, 첫 세션까지 안내하는 체크리스트
- 🔔 **알림 센터** — 완료 알림이 재시작 후에도 유지되고, 미확인/확인함 탭에서 클릭하면 해당 채팅으로 바로 이동
- 📊 **AI 사용량 대시보드** — 머신별 Claude·Codex 남은 사용량(5시간/7일 창), Claude 계정 원클릭 전환
- ⌨️ **단축키 커스텀 & 원클릭 분할** — 모든 키를 재바인딩, 어느 패널 옆이든 터미널(⌘⌥T)·채팅(⌘⌥C) 분할, 탭 우클릭 메뉴
- 🎙️ **실시간 음성 입력 & 파일 첨부** — 말하는 대로 실시간 전사, 파일·이미지를 끌어다 첨부
- 📄 **리치 파일 뷰어** — 마크다운·HTML·PDF·DOCX를 앱 안에서 바로 렌더링
- 🔗 **GitHub / GitLab 가져오기** — 계정을 연결하면 프로젝트 생성 시 저장소를 목록에서 선택
