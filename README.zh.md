...
**企业级氛围编程与交付平台 (Enterprise Vibe Coding & Delivery Platform)** — 用自然语言描述你想要的东西,
AI 智能体负责构建并实时预览;成果由公司部署、共享、拥有。
...
### [⬇️ 下载 macOS 版 (Apple Silicon)](https://github.com/buzzni/saycode-desktop-releases/releases/download/v0.1.38/Saycode-0.1.38-arm64.dmg)
...
## 这和 Orca 这类 Agent IDE 有什么不同？

像 [Orca](https://www.onorca.dev) 这样的工具是 **Agent IDE**,专注于开发者的编码循环:
跨 worktree 并行运行多个智能体、diff、代码评审、深度编辑器集成。它们在这方面做得很好 ——
其中不少也提供组织级的推广和治理能力 —— 如果这正是你要解决的问题,选它就好。

Saycode 瞄准的是一个不同的问题 —— 从智能体完成工作*之后*才开始的问题:

> **Agent IDE 让开发者运行更多智能体。
> Saycode 让公司拥有并运营这些智能体构建的成果。**

| | Agent IDE(Orca 等) | **Saycode** |
|---|---|---|
| 主要关注点 | 开发者的编码循环 | **公司的交付循环** — 构建 → 部署 → 共享 → 交接 |
| 工作单元 | 仓库·worktree·智能体会话 | **组织·团队·项目·机器·部署** |
| “部署”的含义 | 把智能体工具安全地推广到你的组织 | **把构建成果发布到团队可访问的内网 URL** |
| 典型终点 | 评审通过的改动合并进 Git | **一个正在运行、可共享、可交接的内部服务** |
| AI 模型成本 | 用量查看、账号切换 | **按难度自动路由 + 子智能体委派 — 已有组织整体节省近 50%** |
| 触达范围 | 主要是开发者 | **公司里的每个人** — 开发者、产品经理、运维 |

两者都能做到并行智能体、worktree、远程执行以及本地优先的安全性 — 这些不是你该
选择哪一个的理由。Saycode 真正增加的是在此之上的一层**运营层**:

- **公司所有权** — 项目、会话、规划文档和机器都归属于组织,而不是归属于
  开发它们的那台个人电脑。当有人离职或调岗时,他们的工作成果依然可搜索、可交接,
  而不会被搁置。
- **内网部署** — 一键发布到稳定的内网 URL(自动 SSL)。产出的不是一个
  等待评审的分支,而是团队今天就能打开使用的运行中的工具。
- **共享与交接** — 同事可以浏览、克隆、优化并合并回去。别的团队做完的项目
  是一个新的起点,而不需要从头重建。
- **成本运营** — 请求会自动路由到成本最低但能胜任的模型(只有会话卡住时才升级),
  机械性的子任务会委派给使用轻量模型的子智能体,而这一切都运行在集中管理的组织
  共用机器上。以这种方式运营的组织,整体 AI Token 支出最多可节省近 50% —
  每条消息都用徽章标明为什么选择了这个模型。你管理的是全公司的 AI 支出,
  而不只是旁观它。

**该用哪一个:** 如果你的目标是在自己的代码仓库中获得最深度的并行智能体编码体验,
Agent IDE 是不错的选择 — 而且没有什么能阻止你两者都用。如果你的目标是让 AI 构建的
软件成为**公司的基础设施** — 在内网部署、跨团队共享、经得起人员交接、成本可控 —
那正是 Saycode 的用武之地。
...
清楚省了多少、为什么选择了这个模型。在公司层面效果还会叠加:把机械性的子任务
委派给轻量模型的子智能体,并在组织共用机器上集中管理用量,已有组织把整体
AI Token 支出削减了近 50%。
...
右键点击任意标签页即可拆分出新的聊天或终端窗格,拖动即可调整大小 — 工作区随手势
即时重排。在一个窗格里看 Claude 思考,另一个窗格里看 Codex 交付。把任务分发给多个智能体后,
点击**全部打开**即可一次性把所有会话排成网格。侧边栏实时追踪每一个正在运行的智能体。
