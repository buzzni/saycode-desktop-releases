...
**エンタープライズ・バイブコーディング＆デリバリープラットフォーム (Enterprise Vibe Coding & Delivery Platform)** —
作りたいものを自然な言葉で伝えるだけで、AI エージェントが作ってライブでプレビュー。
その成果物は、会社が展開し、共有し、所有します。
...
### [⬇️ macOS 版をダウンロード (Apple Silicon)](https://github.com/buzzni/saycode-desktop-releases/releases/download/v0.1.25/Saycode-0.1.25-arm64.dmg)
...
## エージェント IDE（Orca など）との違いは？

[Orca](https://www.onorca.dev) のようなツールは **エージェント IDE** であり、開発者の
コーディングループ — worktree・差分表示・レビュー・エディタとの深い連携を通じて複数のエージェントを
並行して動かすこと — に重点を置いています。そこは非常に得意としており、中にはロールベースの
チーム展開など組織レベルの導入・ガバナンスを提供するものもあります。それが解決したい課題なら、
ぜひ使うべきです。

Saycode が向き合うのは別の問いです。エージェントの作業が *終わった後* に始まる問いです。

> **エージェント IDE は開発者がより多くのエージェントを動かすためのもの。
> Saycode はそのエージェントが作った成果物を会社が所有・運用するためのもの。**

| | エージェント IDE（Orca など） | **Saycode** |
|---|---|---|
| 主眼 | 開発者のコーディングループ | **会社のデリバリーループ** — ビルド → デプロイ → 共有 → 引き継ぎ |
| 作業の単位 | リポジトリ・worktree・エージェントセッション | **組織・チーム・プロジェクト・マシン・デプロイ** |
| 「デプロイ」の意味 | エージェントツールを組織へ安全に展開すること | **作ったものを、チームが開く社内 URL に公開すること** |
| 典型的な終着点 | レビュー済みの変更が Git にマージされること | **共有・引き継ぎ可能な、稼働中の社内サービス** |
| AI モデル費用 | 使用量の確認、アカウント切り替え | **難易度に応じた自動ルーティング + サブエージェント委任 — 組織全体で ~50% 削減した例も** |
| 届く範囲 | 主に開発者 | **会社の全員** — 開発者、PM、運用担当まで |

両者は並行エージェント、worktree、リモート実行、ローカルファーストのセキュリティという点で
重なっており、そこはどちらを選ぶ決め手にはなりません。Saycode が上乗せするのは
**運用レイヤー** です。

- **会社としての所有** — プロジェクト、セッション、企画ドキュメント、マシンは組織のものであり、
  誰の PC で作られたかには依存しません。誰かが退職・異動しても、その成果は検索・引き継ぎ可能な
  ままで、置き去りにはなりません。
- **社内デプロイ** — ワンクリックで安定した社内 URL に公開されます（SSL 自動）。成果物は
  レビュー待ちのブランチではなく、チームが今日から開ける稼働中のツールです。
- **共有・引き継ぎ** — 同僚が閲覧・複製・改良し、マージし直せます。他チームの完成した
  プロジェクトは、作り直す対象ではなく、次の出発点になります。
- **コスト運用** — リクエストは自動で最も安価に処理できるモデルへルーティングされ
  （セッションが行き詰まったときだけエスカレーション）、機械的なサブタスクは軽量モデルの
  サブエージェントに委任され、そのすべてが中央管理された組織共用マシンの上で動きます。
  この形で運用した組織では、AI トークン費用全体を 50% 近く削減できた例もあります —
  メッセージごとに、なぜそのモデルが選ばれたのかがバッジで示されます。会社の AI 予算を、
  見ているだけでなく管理できます。

**どちらを使うべきか:** 自分のリポジトリで並行エージェントコーディングを可能な限り極めたいなら、
エージェント IDE も良い選択です — そして両方を併用することを妨げるものは何もありません。AI が
作ったソフトウェアを **会社のインフラ** にしたいなら — 社内でデプロイされ、チーム間で共有され、
引き継ぎを経ても残り続け、コストも管理できるものにしたいなら — それが Saycode の役割です。
...
いつでもわかります。会社単位ではさらに効果が積み上がります: 機械的なサブタスクを軽量モデルの
サブエージェントに委任し、組織共用マシンで使用量を中央管理することで、AI トークン費用全体を
50% 近く削減した組織もあります。
